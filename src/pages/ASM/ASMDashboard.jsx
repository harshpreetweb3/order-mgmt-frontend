...
export const ASMDashboard = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      showError(err.message || 'Failed to fetch ASM orders');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        >
          ...
          <span>Create Order to SS / Company</span>
        </button>
      </div>
              ...
              {loading ? (
                ...
                orders.map((ord) => (
                  <tr key={ord._id} className="transition-colors hover:bg-slate-800/20">
                    <td className="p-3 sm:px-4 font-bold text-purple-400">{ord.orderNumber}</td>
