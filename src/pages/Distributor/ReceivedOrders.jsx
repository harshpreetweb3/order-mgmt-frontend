...
export const ReceivedOrders = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      showError(err.message || 'Failed to fetch received orders');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMarkDelivered = async (ord) => {
    try {
      ...
      showSuccess(`Order #${ord.orderNumber} marked as Delivered successfully!`);
      fetchOrders();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-white tracking-tight">Orders Received from Salesmen</h1>
        <p className="text-sm text-slate-400">View orders placed by salesmen and process delivery status</p>
      </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="text-center py-12 text-slate-500">
                    No received orders match your criteria.
                  </td>
                </tr>
              ) : (
                orders.map((ord) => (
